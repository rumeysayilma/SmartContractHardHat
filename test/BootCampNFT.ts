...
describe("Bootcamp NFT Contract", () => {
  ...
  describe("Others", () => {
    ...
    it("is Award Item Correct Minted", async () => {
        await bootcampNFT.awardItem(FIRST.address, "veliuysal.dev");
        expect(await bootcampNFT.ownerOf(Number(0))).to.equal(FIRST.address); //0 Numaralı nft nin sahibi first adress mi?
      });
      //HER İTERASYON BİRBİRİNDEN BAGIMSIZ CALISIR
  });
});